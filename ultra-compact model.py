...
def resize_image(image, size=(12, 12)):
    # Resize the image to fit N x N pixels (12x12)
...
def process_data(x_data, y_data, size=(12, 12), m_bits=1):
    processed_images = []
    labels = []
...
def build_model(input_shape=(12, 12, 1)):
    ...
    model.add(tf.keras.layers.Dropout(0.01))  # 20% dropout to avoid overfitting
...
def train_and_evaluate():
    ...
    x_train_processed, y_train_processed = process_data(x_train, y_train, size=(12, 12), m_bits=3)
    x_test_processed, y_test_processed = process_data(x_test, y_test, size=(12, 12), m_bits=3)
    ...
    # Build the model
    model = build_model(input_shape=(12, 12, 1))

    # Train the model
    model.fit(x_train_simplified, y_train_simplified, epochs=30, batch_size=64, validation_data=(x_test_simplified, y_test_simplified), verbose=2)
